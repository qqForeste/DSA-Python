class Solution:
    def solve(self, board: List[List[str]]) -> None:
        ...
        for r in range(ROWS):
            ...
            if board[r][0] == 'O' and (r, 0) not in notsurrounded:
                BFS(r, 0, notsurrounded)
            if board[r][COLS - 1] == 'O' and (r, COLS - 1) not in notsurrounded:
                BFS(r, COLS - 1, notsurrounded)
        
        for c in range(COLS):
            
            if board[0][c] == 'O' and (0, c) not in notsurrounded:
                BFS(0, c, notsurrounded)
            if board[ROWS - 1][c] == 'O' and (ROWS - 1, c) not in notsurrounded:
